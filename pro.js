...
(function () {
    ...
    async function useStoredRequests(storedRequests, total) {
        var allres = [];
        ...
        if (n100 !== 0) {
            ...
            for (let i = 1; i <= n100; i++) {
                ...
                let res = await customAjaxPost(url, modifiedBody, HeadersCopy).then(response => {
        ...
        let filename = JSON.parse(body0).unitName + ".csv";
        if (remainder <= 10 && n100 == 0) {
            ...
            const blob = new Blob(["\uFEFF" + csv], { type: 'text/csv;charset=GB18030' });
        ...
        if (!requestLookup.includes(requestId) && event.type === 'load' && event.target.readyState === 4) {
            ...
            if (event.target.responseText.includes('total') && event.target.responseText.includes('hasNextPage')) {
                ...
                if (event.target.responseText.includes("domainId")) {
                    ...
                    var total = res.params.total;
                    useStoredRequests(storedRequests, total);
                }
            }
        };
    }
